...
Can the trained model be used to classify records with an uncertain label (especially the `data_type` "Other" category, 3,544 records)? Are there hidden sensitive parameters mislabeled as "Other" — records that actually collect sensitive data but weren't tagged as such?
...
- **7 hidden sensitive records** were found: parameters confidently point to a **sensitive category** (Security credentials or Personal information at the main_data_type level) despite being labeled "Other" in the original dataset. Examples: `name="email"` (91% confidence → Email address), `name="key"`/`name="KEY"` (82% confidence → API key), `name="token"` (51% confidence → Access tokens). Their names alone make it obvious they're sensitive data — this is the clearest evidence of hidden/mislabeled sensitive parameters in the dataset.
