...
A sixth, supplementary finding — whether plugins disclose what they collect in their actual privacy policies — comes from a separate audit dataset (see **Data Sources & Attribution** below): across 308 comparable parameters, **90.6% are never disclosed** in the plugin's privacy policy at all. Note that the source paper labels disclosure with 5 categories (CLEAR/VAGUE/AMBIGUOUS/INCORRECT/OMITTED); the published `backend/final_results/` data used here contains no `AMBIGUOUS`-labeled entries, so this project's audit works with the remaining 4 categories.
