...
- The chatbot's LLM answers are now rendered from explicit **fact IDs** rather than free-form numbers — if the model states a number that isn't backed by a recognized fact ID, the answer is rejected rather than shown to the user (see `backend/app/facts.py::render_grounded_answer`).
...
- **LLM**: Anthropic Claude, used only to phrase the answer from retrieved context and to select which fact IDs support it — never to invent or compute numbers on its own (see Validation & Reproducibility above).
...
- **Numeric grounding**: `backend/app/project_facts.json` holds every verified statistic from the analysis, generated directly by `analysis/rebuild.py` rather than hand-typed. Answers are rendered from explicit fact IDs, and an answer citing an unrecognized fact ID or an unsupported free-form number is rejected rather than shown to the user.
...
- **The chatbot's charts are reliable** — they're static images exported directly from the notebooks' actual output, not generated on the fly. The LLM-generated explanatory *text* is now structurally grounded to fact IDs, but that mechanism guarantees numeric traceability, not that every qualitative claim is correct — as a general principle for any RAG-based system, claims should be cross-checked against the source data before being relied on for a decision or publication.
